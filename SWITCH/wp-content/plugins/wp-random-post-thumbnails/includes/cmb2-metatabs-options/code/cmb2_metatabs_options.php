...
class Cmb2_Metatabs_Options {
	...
	private function add_wp_actions( $id ) {
		...
		if ( self::$props[ $id ]['regkey'] ) {
			add_action(
				'admin_init',
				function () use ( $id, $_this ) {
					$_this->register_setting( $id );
				}
			);
		}
		...
		add_action(
			...
			function () use ( $id, $_this ) {
				...
				$args = func_get_args();
				echo $_this->render_save_button( $args[0]->args['desc'] );
			}, 10, 1 );
	}
	...
	private function build_menu_args( $id ) {
		...
		$args[] = function () use ( $id, $_this ) {
			$_this->admin_page_display( $id );
		};
	...
	public function add_metaboxes( $id ) {
		...
		foreach ( self::$props[ $id ]['boxes'] as $box ) {
			...
			add_action( 'cmb2_save_options-page_fields_' . $mid,
				function () use ( $id, $_this ) {
					$_this->settings_notices( func_get_args(), $id );
				}, 10, 2 );
			...
			add_meta_box(
				...
				$box->meta_box['title'],
				function () use ( $id, $_this ) {
					$_this->metabox_callback( func_get_args(), $id );
				},
				self::$props[ $id ]['hook'],
